...
const fetchWord = async (word: string): Promise<WordData | null> => {
	...
	if (response.ok && response.body !== null) {
		...
		console.error(
			`Backend returned ${response.status}: ${await response
				.text()}`,
		);
		return null;
	}
...
const fallBackHand = [...Array(21).keys().map((_) => [5, 5, 5])];
const fallBackFace = [...Array(468).keys().map((_) => [5, 5, 5])];
...
const prepareAnim = (
	...
	return () => {
		...
		if (currFrame >= currWordData.length) {
			...
			// Still on this word
			const currFrameData =
				currWordData[animContext.currentFrame];
