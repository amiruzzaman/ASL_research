...
def inference(args):
    ...
    while True:
        ...
        tokens = torch.tensor(
            [
                gloss_vocab[word if word in gloss_vocab else "<unk>"]
                for word in sequence.split()
            ]
        ).to(DEVICE)
        tokens = torch.cat(
            [
                torch.tensor([gloss_vocab["<sos>"]]).to(DEVICE),
                tokens,
                torch.tensor([gloss_vocab["<eos>"]]).to(DEVICE),
            ]
        ).to(DEVICE)
...
def train(args):
    train_dl, test_dl, gloss_vocab, gloss_id, text_vocab, text_id = load_data(
        args.batch
    )
    ...
    for epoch in range(curr_epoch, EPOCHS + 1):
        ...
        valid_loss, correct = validate(
            model, test_dl, criterion, gloss_vocab, text_vocab
        )
