...
class EnglishToGloss:
    def __init__(self):
        ...
        self.model = TranslatorModel(len(self.text_vocab), len(self.gloss_vocab), 512, 8, 2, 2, 0.3, device=DEVICE)
        weights = torch.load(os.path.join(PATH, 'english_to_gloss', 'best.pt'), weights_only=False)
