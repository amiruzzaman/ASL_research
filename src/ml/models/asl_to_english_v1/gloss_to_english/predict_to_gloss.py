...
def inference(args):
    ...
    model = TranslatorModel(len(text_vocab), len(gloss_vocab), args.dmodel, args.heads, args.encoders, args.decoders, args.dropout, device=DEVICE).to(DEVICE)
    ...
    while True:
        ...
        tokens = convert_to_tokens(sequence, text_vocab, DEVICE)
        ...
        if args.greedy:
            translated_tokens = model.greedy_decode(tokens, mask, text_vocab, gloss_vocab, device=DEVICE).flatten()
        else:
            translated_tokens = model.beam_search(tokens, mask, text_vocab, gloss_vocab, device=DEVICE, beam_size=args.beam_size, max_len=num_tokens + 5, temperature=0.5).flatten()
        
        translated = " ".join([gloss_id[token] for token in translated_tokens.tolist()]).replace("<sos>", "").replace("<eos>", "")
