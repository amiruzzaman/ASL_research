...
class Trainer:
    def __init__(
        ...
        self.ctc_loss = nn.CTCLoss(blank=self.gloss_to_idx["-"], zero_infinity=True).to(
            gpu_id
        )
        self.cross_entropy_loss = nn.CrossEntropyLoss(
            ignore_index=self.word_to_idx["<pad>"]
        ).to(gpu_id)
    ...
    def train(self):
        ...
        if self.gpu_id == 0:
            print(
                f"Starting Average Gloss Loss: {valid_recognition_loss.item():.4f}",
                end=" - ",
            )
            print(
                f"Starting Average Sentence Loss: {valid_translation_loss.item():4f}",
                end=" - ",
            )
        ...
        for epoch in range(self.curr_epoch, self.epochs + 1):
            ...
            train_recognition_loss, train_translation_loss, train_loss = (
            ...
            if self.gpu_id == 0:
                ...
                print(
                    f"Training Average Gloss Loss: {train_recognition_loss.item():.4f}",
                    end=" - ",
                )
                print(
            ...
            if epoch % self.validate_every == 0:
                ...
                if self.gpu_id == 0:
                    ...
                    # Showing metrics
                    print(
                        f"Valid Average Gloss Loss: {valid_recognition_loss.item():.4f}",
                        end=" - ",
                    )
                    print(
    ...
    def _train_epoch(self, epoch: int = 1):
        ...
        dl = (
            self.train_dl
            if self.gpu_id != 0
            else tqdm(self.train_dl, desc=f"Epoch {epoch}")
        )
        ...
        for videos, video_lengths, glosses, gloss_lengths, sentences, _ in dl:
            ...
            translation_loss = (
                self.cross_entropy_loss(actual, expected) * self.translation_weight
            )
    ...
    def _validate(self, epoch: int = 1):
        ...
        for (
            ...
            actual_gloss = decode_glosses(
                glosses.tolist(), self.gloss_to_idx, self.idx_to_gloss
            )
            predicted_gloss = decode_glosses(
                encoder_out, self.gloss_to_idx, self.idx_to_gloss
            )
        ...
        predicted_glosses = list(
            itertools.chain.from_iterable(gathered_predicted_glosses)
        )
        actual_glosses = list(itertools.chain.from_iterable(gathered_actual_glosses))
        predicted_sentences = list(
            itertools.chain.from_iterable(gathered_predicted_sentences)
        )
        actual_sentences = list(
            itertools.chain.from_iterable(gathered_actual_sentences)
        )
...
def start_training(rank: int, world_size: int, config: dict):
    ...
    vocab, train_dl, valid_dl, test_dl = create_dataloaders(
        PROCESSED_PATH, training_config
    )
